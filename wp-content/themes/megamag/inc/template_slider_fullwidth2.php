							...
							$cat = get_the_category($results_slider_posts[$i]->ID);
								...
								<div id="htmlcaption<?php echo $i; ?>" class="nivo-html-caption">
									<div class="featured-text">
										<h2><a href="<?php echo get_permalink($results_slider_posts[$i]->ID); ?>"><?php if (empty($result_cmb_hide_slider_caption_title)) {echo $result_cmb_slider_caption_title;} ?></a></h2>
										<span class="slider-meta"><?php echo mb_localize_datetime(format_datetime_str(get_option('date_format'), $results_slider_posts[$i]->post_date));?> <a href="<?php echo get_category_link($cat[0]->term_id ); ?>"><?php echo $cat[0]->cat_name; ?></a></span>
										<p><?php if (empty($result_cmb_hide_slider_caption)) {echo $result_cmb_slider_caption;} ?></p>
									</div>
								</div>
								<?php
						}
