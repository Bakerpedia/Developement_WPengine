<?php  /* Template Name: Ingredients */ ?>

<?php 
  /* used for the alphabetical listing include */
  $category = 38;
?>

<?php get_header(); ?>
  
  <?php $megamag_options_post = get_option('megamag_options_post') ?>
		
		<div id="main" class="container">		
			<div id="content">					
      <!-- 
      get featured Ingredients post (CUSTOM KEVIN H.)
      -->		
			<?php
			//display 1 posts for category id 38
				$args=array(
				  'cat' => 38,
				  'post_type' => 'post',
				  'post_status' => 'publish',
				  'posts_per_page' => 1,
				  'caller_get_posts'=> 1
				  );
				$my_query = null;
				$my_query = new WP_Query($args);
        
        ///Summary
        // render Latest Ingredients
        // placed right under the menu   
				if( $my_query->have_posts() ) {                    
				  echo '<h1 class="blog-heading">Latest Ingredients Post</h1>';
				  while ($my_query->have_posts()) : $my_query->the_post(); ?>
				    <div class="blog-item" id="column1-wrap">	
					  <div class="archive-text" style="padding-bottom:5px;" >
						  <a href="<?php the_permalink(); ?>"><?php the_title( '<h1>', '</h1>' ); ?></a>
						  <a title="<?php the_title(); ?>" href="<?php the_permalink(); ?>">
							  <?php echo get_the_post_thumbnail($page->ID, 'medium'); ?>
						  </a>
					  </div>
				  </div>
				    <div id="column2" class="blog-item">				
					    <?php					
						    $key = "definition";
						    $thumb = get_post_meta(get_the_ID(), $key, true);
						  
                if ($thumb) {
							    echo "<strong>Definition</strong>";
							    echo "<div>" . $thumb . "</div>";
						    }
					    ?>
					    </br><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="readmore">Read More</a>
				    </div>        
				<?php
				  endwhile;
				}
			wp_reset_query();  // Restore global post data stomped by the_post().
			?>
		<!-- 
    END get featured Ingredients post (CUSTOM KEVIN H.)
    -->	
			
				<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
				  <div class="post">
				  <?php 

					if (isset($megamag_options_post['show_featured'])) {
						$thumbnail_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full');
						if (get_the_post_thumbnail() != NULL) { 
							?>
            <img src=""<?php printf($thumbnail_url[0]); ?> width=610></img>
						<?php
						}
					}
				?>

					<h1 class="post-heading page"><?php the_title(); ?></h1>					
					  <div class="post-content">						
						  <?php the_content(); ?>
					  </div>					
					  <?php 
						  //if (isset($megamag_options_post['show_comments'])) {
						  //	comments_template( '', true ); 
						  //}
					  ?>
					
				</div>
				<!-- END POST -->

				<?php endwhile; else: ?>
				<p><?php _e('Sorry, no posts matched your criteria.', 'lcz_megamag'); ?></p>
				<?php endif; ?>	<!-- main post loop -->

			<!---NEW alpha listing include 02/24 KJH-->
       <?php include('AlphaListingInclude.php'); ?>

			</div>
			<!-- END CONTENT -->

<?php get_sidebar(); ?>			

<?php get_footer(); ?>



<script type="text/javascript">
  $(document).ready(function () {
  $(".post-heading.page").hide();
  });
</script>
