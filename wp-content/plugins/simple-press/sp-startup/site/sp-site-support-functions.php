...
$LastChangedDate: 2014-08-27 02:08:58 -0700 (Wed, 27 Aug 2014) $
$Rev: 11905 $
*/
...
# Attempt to grab the users actual language country code
# This will try and match it against our internal list
# ------------------------------------------------------------------
...
function sp_set_image_array($curTheme) {
	...
	if(isset($curTheme['icons']) && !empty($curTheme['icons'])) {
	...
	if(!empty($curTheme['parent'])) {
		...
		if(isset($curTheme['icons']) && !empty($curTheme['icons'])) {
			$p = ($spDevice == 'mobile') ? $curTheme['parent'].'/'.$curTheme['icons'].'/mobile/' : $curTheme['parent'].'/'.$curTheme['icons'].'/';
			$spImages[$idx]['dir'] = SPTHEMEBASEDIR.$p;
			$spImages[$idx]['url'] = SPTHEMEBASEURL.$p;
			$idx++;
		}
