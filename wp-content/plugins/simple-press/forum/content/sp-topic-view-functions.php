...
$LastChangedDate: 2014-07-18 08:43:57 -0700 (Fri, 18 Jul 2014) $
$Rev: 11776 $
*/
...
function sp_PostNewButton($args='', $label='', $toolTip='', $toolTipLock='', $toolTipDenied='') {
	...
	if ($spGlobals['lockdown'] || $spThisTopic->forum_status || $spThisTopic->topic_status) {
		if (!empty($iconLock)) {
			...
			$out.= "<img class='$tagClass $iconStatusClass vtip' id='$tagId' src='$iconLock' title='$toolTipLock' alt='' />\n";
		}
	...
	if (!$allowed && !empty($toolTipDenied)) {
		if (!empty($iconDenied)) {
			...
			$out.= "<img class='$iconStatusClass vtip' src='$iconDenied' title='$toolTipDenied' alt='' />\n";
			$out.= "</a>\n";
		}
	}
...
function sp_PostNewTopicButton($args='', $label='', $toolTip='', $toolTipLock='') {
	...
	if ($spGlobals['lockdown'] || $spThisTopic->forum_status) {
		if (!empty($iconLock)) {
			$iconLock = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconLock));
			$out.= "<img class='vtip $tagClass $iconStatusClass' src='$iconLock' title='$toolTipLock' alt='' />\n";
		}
        if (!$spThisUser->admin) $lock = true;
	}
...
function sp_PostIndexPinned($args='', $toolTip='') {
	...
	if (!empty($icon)) $out.= "<img class='vtip' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' title='$toolTip' alt='' />\n";
	$out.= "</span>";
...
function sp_PostIndexUserTwitter($args='', $toolTip='') {
	...
	if (!empty($icon)) {
		$out = "<a id='$tagId' class='$tagClass vtip' href='$url' title='$toolTip'$target$follow><img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' /></a>\n";
	}
...
function sp_PostIndexUserWebsite($args='', $toolTip='') {
    ...
    $follow = ($noFollow) ? ' rel="nofollow"' : '';
	if (!empty($icon)) {
		$out = "<a id='$tagId' class='$tagClass vtip' href='$spThisPostUser->user_url' title='$toolTip'$target$follow><img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' /></a>\n";
	}
