...
function sp_do_sp_RecentPostsTag($args='') {
	...
	if (sp_has_list()) : while(sp_loop_list()) : sp_the_list();
		...
		if ($aLink) $out.= "<a class='$linkClass' $title href='$spThisListTopic->post_permalink'>";
		...
		for ($x=0; $x<strlen($itemOrder); $x++) {
			...
			switch (substr($itemOrder, $x, 1)) {
				case 'F':
					# Forum
					$out.= $beforeForum;
					if ($fLink) $out.= "<a class='$linkClass' href='$spThisListTopic->forum_permalink'>";
